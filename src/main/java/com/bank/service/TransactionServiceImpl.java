...
public class TransactionServiceImpl implements TransactionService {
	...
	public TransactionRespDto transferMoneyByCard(TransactionByCardReqDto transaction) {
		...
		Card cd = cardDao.findById(transaction.getCardNumber())
		...
		if (cd.getCVV() == transaction.getCvv() && transaction.getCardNetwork().equals(cd.getCardNetwork().name())
			...
			if (!senderAcc.getPassword().equals(transaction.getPassword())) {
				throw new InvalidPasswordException("Wrong Password!!!");
			}
			if (senderAcc.getBalance() < transaction.getAmount()) {
				throw new LimitExceededException("insufficient balance");
			}
			Account recieverAcc = accountDao.findById(transaction.getAccountNo()).orElseThrow(
					() -> new BranchNotFoundException("Account not found, invalid reciever's account number!!!"));
			...
			senderAcc.addDebitTransaction(transactions);
			recieverAcc.addCreditTransactions(transactions);
			transactions.setTransactionType(TransactionType.valueOf("DEBIT"));
			transactions.setTransactionDate(LocalDate.now());
